...
import com.spaghetti.assets.JarFileIO;
...
public class ModelLoader implements AssetLoader<Model> {
	...
	public Object[] load(String[] args) throws Throwable {
		...
		InputStream data_stream = ResourceLoader.getStream(args[0]);
		...
		// Import
		AIScene scene = Assimp.aiImportFileEx(args[0],
		...
		if (args[1].equals("find")) {
			...
			for (AIMesh mesh : meshes) {
				if (ExceptionUtil.getAssimpString(mesh.mName()).equals(args[2])) {
			...
			if (!found) {
				throw new IllegalArgumentException("Couldn't find mesh " + args[2]);
			}
		} else if (args[1].equals("unify")) {
	...
	public String[] getDefaultArgs() {
		return new String[] {"/internal/square.obj", "find", "square"};
	}

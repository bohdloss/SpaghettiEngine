...
public final class EventDispatcher {
	...
	// Events can only be raised on the server side of a game and are transmitted to
	// all clients
	// Events can also be caught by event handlers on both the server and the
	// clients
	public void raiseEvent(GameObject issuer, GameEvent event) {
		if (!issuer.getGame().hasAuthority()) {
			throw new UnsupportedOperationException(
					"You don't have authority to raise events (You must be a server or a singleplayer game instance)");
		}

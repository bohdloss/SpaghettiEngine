...
public final class AssetLoader {
	...
	private static void loadObj(Model model, SheetEntry data) throws Throwable {
		...
		/*
		 * Get a face from the list, get a certain vertex index from it,
		 * and from it get the vertex index [0] and the texture index[0]
		 */
		...
		for(int i = 0; i < faces.size(); i++) {
			...
			for(int j = 0; j < facedata.length; j++) {
				...
				int vindex = facedata[j][0];
				int tindex = facedata[j][1];
				int nindex = facedata[j][2];
	...
	public static void loadShaderProgram(AssetManager manager, ShaderProgram shaderProgram,
			SheetEntry data) throws Throwable {
